      ...
      return new Promise( (resolve, reject) => {
        let mapping = jmespath.search(interfaces,
            `NetworkInterfaces[].{
              ...
              securityGroupName: Groups[].GroupName,
...
const decorateRecords = (records, mapping) => {
  ...
  records.forEach( (record) => {
    ...
    if (match) {
        ...
        if (eniData) {
          let sg_id_name = eniData.securityGroupIds;
          console.log("security group name "+eniData.securityGroupName)
          let gName = eniData.securityGroupName.toString()
          if(gName.indexOf('-') > 0)
            sg_id_name = eniData.securityGroupIds + '-' + gName.split("-")[1];
